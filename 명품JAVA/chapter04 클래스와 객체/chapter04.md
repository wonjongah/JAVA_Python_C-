...
자바에서 기본 타입 데이터뿐 아니라, 객체를 원소로 하는 객체 배열도 만들 수 있다.

C/C++와 달리, 자바의 객체 배열은 객체에 대한 레퍼런스를 원소로 갖는 배열이다.

```java
Circle[] c; // Circle 배열의 레퍼런스 변수 c 선언
...
앞의 코드는 5개의 Circle 객체들을 원소로 저장하는 배열을 만든 예이다.

아래의 코드는 배열에 들어 있는 모든 Circle 객체의 면적을 출력하는 코드 사례이다.
...
Circle 클래스의 배열의 레퍼런스 변수 c를 선언한다. 레퍼런스 변수 c만 선언했을 뿐, 배열을 생성한 것은 아니다. 그러므로 배열 원소 개수를 지정해선 안 된다.

```java
Circle[5] c; // error, 아직 배열이 생성되지 않았는데 배열의 크기 지정하면 컴파일 오류
```
